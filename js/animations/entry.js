...
/**
 * Entry 섹션 애니메이션을 초기화하는 함수 (intro_new.js의 sectionEntry 1:1 이관)
 */
export function initEntryAnimation() {
    ...
    .to(words[0], { [motionDir]: getResponsiveValue(0, -93), opacity: 1, duration: 1, ...EASING }, '<')
    .addLabel('word_change1')
    .to($firstWord, { [motionDir]: getResponsiveValue(FIRST_WORD_START_MOBILE, -53), [otherDir]: 0, duration: 1, ...EASING }, '+=2')
    .to(words[0], { [motionDir]: DISAPPEAR, opacity: 0, ...EASING }, '<')
    .to(words[1], { [motionDir]: APPEAR, opacity: 1, duration: 1, ...EASING }, '<')
    .addLabel('word_change2')
    .to(words[1], { [motionDir]: DISAPPEAR, opacity: 0, ...EASING }, '+=2')
    .to(words[2], { [motionDir]: APPEAR, opacity: 1, duration: 1, ...EASING }, '<')
    .addLabel('word_change3')
    .to(words[2], { [motionDir]: DISAPPEAR, opacity: 0, ...EASING }, '+=2')
    .to(words[3], { [motionDir]: APPEAR, opacity: 1, duration: 1, ...EASING }, '<')
    .addLabel('disappear')
